...
import { channelEpic } from './channel';
	...
	action$.pipe(
		...
		mergeMap(action => {
			return interval(3000).pipe(
				...
				takeUntil(
					action$.ofType(types.STOP_FETCHING_CHARACTERS, types.FETCH_CHARACTER_FAILURE),
				),
			);
		}),
	);
	...
	action$.pipe(
		...
		mergeMap(action =>
			request({
				...
				catchError(error =>
					of(actions.fetchCharacterFailure(error.xhr.response, action.payload.isServer)),
				),
			),
		),
	);
